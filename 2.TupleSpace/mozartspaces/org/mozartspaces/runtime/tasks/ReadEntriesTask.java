...
/**
 * A <code>ReadEntriesTask</code> reads entries from a container. It calls the
 * CAPI3 method to read entries,
 * {@link org.mozartspaces.capi3.Capi3#executeReadOperation(LocalContainerReference,
 * java.util.List, org.mozartspaces.capi3.IsolationLevel, SubTransaction,
 * org.mozartspaces.core.RequestContext) executeReadOperation}, with the
 ...
 */
@NotThreadSafe
public final class ReadEntriesTask<T extends Serializable> extends TransactionalTask<ArrayList<T>> {

    private static final Logger log = LoggerFactory.get();

    private final ReadEntriesRequest<T> request;
    private final AspectInvoker aspectInvoker;
    private final Capi3 capi3;

    /**
     * Constructs a <code>ReadEntriesTask</code>.
     *
    ...
    public ReadEntriesTask(final RequestMessage requestMessage, final RuntimeData runtimeData) {
        super(requestMessage, runtimeData);
        this.request = (ReadEntriesRequest<T>) requestMessage.getContent();
    ...
    protected ArrayList<T> runInSubtransaction(final Transaction tx, final SubTransaction stx) throws Throwable {
        ...
        log.debug("Reading from container {}", request.getContainer());
        ...
        AspectResult result = aspectInvoker.preRead(request, tx, stx, containerCapi3, getExecutionCount());
        ...
        // invoke post-aspects
        result = aspectInvoker.postRead(request, tx, stx, containerCapi3, getExecutionCount(),
