...
/**
 * A <code>TestEntriesTask</code> tests for entries in a container.
 * It calls the CAPI3 method to read entries,
 * {@link org.mozartspaces.capi3.Capi3#executeReadOperation(
 * LocalContainerReference, List, org.mozartspaces.capi3.IsolationLevel,
 * SubTransaction, org.mozartspaces.core.RequestContext) executeReadOperation},
 * with the arguments from the request.
 *
 * @author Tobias Doenz
 */
@NotThreadSafe
public final class TestEntriesTask extends TransactionalTask<Integer> {

    private static final Logger log = LoggerFactory.get();

    private final TestEntriesRequest request;
    private final AspectInvoker aspectInvoker;
    private final Capi3 capi3;

    /**
     * Constructs a <code>TestEntriesTask</code>.
     *
     ...
     */
    public TestEntriesTask(final RequestMessage requestMessage,
            final RuntimeData runtimeData) {
        super(requestMessage, runtimeData);
        this.request = (TestEntriesRequest) requestMessage.getContent();
    ...
    protected Integer runInSubtransaction(final Transaction tx, final SubTransaction stx) throws Throwable {
        ...
        log.debug("Testing in container {}", request.getContainer());
        ...
        AspectResult result = aspectInvoker.preTest(request, tx, stx, containerCapi3, getExecutionCount());
        ...
        int testedEntriesCount = 0;
        if (status != AspectStatus.SKIP) {
            EntryOperationResult opResult = capi3.executeReadOperation(lcref, request.getSelectors(), request
            ...
            case OK:
                ...
                testedEntriesCount = entries.size();
                break;
            default:
        ...
        // invoke post-aspects
        result = aspectInvoker.postTest(request, tx, stx, containerCapi3, getExecutionCount(), entries);
        setAspectEventCategories(containerCapi3.getEventCategories());
        status = result.getStatus();
        switch (status) {
        case OK:
        case SKIP:
            return testedEntriesCount;
        default:
            handleSpecialResult(status.toOperationStatus(), result.getCause(), request.getTimeout());
            return null;
        }
